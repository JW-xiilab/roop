...
def sort_rename_frames(path: str, output_image_format):
    filenames = os.listdir(path)
    filenames.sort()
    for i in range(len(filenames)):
        of = os.path.join(path, filenames[i])
        newidx = i+1
        new_filename = os.path.join(path, f"{newidx:06d}." + output_image_format)
        os.rename(of, new_filename)        


def get_temp_frame_paths(target_path: str, output_image_format) -> List[str]:
    temp_directory_path = get_temp_directory_path(target_path)
    return glob.glob((os.path.join(glob.escape(temp_directory_path), f'*.{output_image_format}')))
...
def replace_template(args, file_path: str, index: int = 0):
    ...
    template = args.CFG.output_template
    replaced_filename = template_parser.parse(template, {
        'index': str(index),
        'file': fn
    })

    return os.path.join(args.output_path, f'{replaced_filename}{ext}')
...
def clean_temp(target_path: str, keep_frames:bool=True) -> None:
    temp_directory_path = get_temp_directory_path(target_path)
...
def prepare_environment(args, output_path=None):
    if output_path:
    ...
    if not args.CFG.use_os_temp_folder:
        os.environ["TEMP"] = os.path.abspath(os.path.join(os.getcwd(), "temp"))
    os.makedirs(os.environ["TEMP"], exist_ok=True)
    os.environ["GRADIO_TEMP_DIR"] = os.environ["TEMP"]
    return output_path

def get_device(execution_providers) -> str:
    if len(execution_providers) < 1:
        execution_providers = ['CPUExecutionProvider']

    prov = execution_providers[0]
