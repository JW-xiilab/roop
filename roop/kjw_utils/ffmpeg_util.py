...
def run_ffmpeg(args: List[str]) -> bool:
    commands = ['ffmpeg', '-hide_banner', '-hwaccel', 'auto', '-y', '-loglevel', args.log_level]
...
def cut_video(original_video: str, cut_video: str, start_frame: int, end_frame: int, video_encoder):
    fps = util.detect_fps(original_video)
    start_time = start_frame / fps
    num_frames = end_frame - start_frame

    run_ffmpeg(['-ss',  str(start_time), '-i', original_video, '-c:v', video_encoder, '-c:a', 'aac', '-frames:v', str(num_frames), cut_video])
...
def extract_frames(target_path : str, trim_frame_start, trim_frame_end, fps : float, output_image_format) -> bool:
    ...
    commands.extend(['-vsync', '0', os.path.join(temp_directory_path, '%06d.' + output_image_format)])
    return run_ffmpeg(commands)


def create_video(args, target_path: str, dest_filename: str, fps: float = 24.0, temp_directory_path: str = None) -> None:
    if temp_directory_path is None:
        temp_directory_path = util.get_temp_directory_path(target_path)
    run_ffmpeg(['-r', str(fps), '-i', os.path.join(temp_directory_path, f'%06d.{args.CFG.output_image_format}'), '-c:v', args.video_encoder, '-crf', str(args.video_quality), '-pix_fmt', 'yuv420p', '-vf', 'colorspace=bt709:iall=bt601-6-625:fast=1', '-y', dest_filename])
